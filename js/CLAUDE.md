...
- **落地的回歸套件在 `tests/`**(repo 內,非 scratchpad):`cd tests && npm i && npm test`(`run-all.mjs`
  自動起 server→逐套件跑→匯總)。核心套件 bottles/wind/oilfire/pickup/ice_slide/onboard/sorting 蓋投擲瓶/風壓/油火/撿道具/冰滑/上手開場/分類事故引擎(需求制+怒氣+暴走);
