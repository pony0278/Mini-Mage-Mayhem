...
| `v2.js` | 389 | glue:輸入 poll(滑鼠/E/J/K/空白+觸控閂鎖)、**step() 迴圈**、渲染橋接(`game.props`/`setGroundMarkers` 每幀重建)、boot、`window.__v2`。**測試旗**:`?grabany=1`(免擊暈隨時舉人+被舉不掙脫)、`?clip=名字`(任意 clip 循環試播+對手 AI 凍結;程式=`__v2.playClip(name)`)、`?slowmo=`、`?avatar=1`、`?fx=low`、`?tune=1` |
