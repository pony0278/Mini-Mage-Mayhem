...
>   掛載端的旋轉/校準是照 GLB proto 的**內部軸向**調的(手套:−z=袖口/+z=指節/+y=手背渦輪,z=最長軸
>   1.78×高)。第一版沿 y 軸搭一根窄管+沒手指 → 同一個掛載旋轉下=綁在手臂上的管子。**先拍 GLB proto
>   三軸純景**(scratchpad/gaunt_axes.mjs 範式:小 scene 從 ±x/±y/±z 拍)再照佈局重建;手套的三個訊息=
>   有手指的拳+手背大渦輪(金環/扇葉/發光轂)+喇叭袖口。剪影+軸系+手指+發光數已鎖進 gauntlet.mjs ②。
