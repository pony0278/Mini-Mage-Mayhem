...
>   千倍=畫面出現巨大白板)→ 夾 `maxGrow`。⑤**線色不能用純隊伍色**(藍描藍=零對比),要往白拉。
>   ⑥細肢只有 6px 寬,線寬 3.2 就等於半條腿=白霧;**1.2 才是乾淨的 contour**。
> 現況:近距離線很乾淨(cel-shading 味),**實戰距離「存在但要注意看」**——恰吉的鏡頭近很多,這是
> 相機距離的差別,不是實作問題。旋鈕 `RIM`(width/tint/alpha/maxGrow)。
