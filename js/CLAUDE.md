...
> 同四步入庫,proto 正中置心非底貼地=裝備繞腕)。actor-brawler `updateGauntlet` 掛 `R.armR.wr`(box 右腕=punch-studio `bow` slot 同一掛點
> → studio 校準值可直搬遊戲 `WIND_CAL`);持風壓手套(`e.item==='wind'`)時顯示、跟手動。clone 網格帶 `__equip` 旗(avatar 藏方塊人掃描跳過)、`__gauntlet` 旗(測試計數)。
> **掛 box 腕不掛 avatar 手骨**:與 studio bow slot 同空間=校準一致(火帽 headgear 同理);avatar 模式的 box腕→avatar手重定向偏差在兩邊一致,使用者在 studio 校準即補償。
