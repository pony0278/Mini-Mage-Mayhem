...
| `fx.js` | ~135 | **共用回饋/格子原語**:addText/addShake/addHitstop/addRing/hitSpark、**`addBurst`(hitfx-1 漫畫打擊爆花→`game.bursts`,參數包=v2-state `HIT_BURST` 分級;v2-hud `drawBursts` 消費)**、update{Particles,Rings,FloatingTexts}、isSolidTile/circleHitsSolid、overVoid/updateDeathTheater。DAG:state→fx→sim;**v2 只依賴 fx,絕不 import sim.js** |
...
| `v2-hud.js` | ~330 | 2D HUD(穩定條/掙脫條/防禦耐力/道具次數/橫幅)+ **`drawBursts`(hitfx-1 漫畫打擊爆花:GetAmped 風白星+彩描邊,最上層蓋角色、3 格幀階跳格;重擊帶黃色速度線+首格白閃、挑飛加全屏集中線;顏色=打擊類型 拳橘/打暈琥珀/反擊金/下壓紅;FX_LOW 留爆花砍線,經 render facade 讀 `FX_LOW`;元素爆炸維持發光粒子=能量感分工)** + **`drawPips`**(左右三格收容進度,填色=收容方式 METHOD_COL)+ **`drawReport`**(結算事故報告全屏卡:等級/標題/統計/封存序列/委員會評語/挑戰碼;R 再戰/C 複製)+ `drawPerformLED`(收容演出艙口 LED)+ `drawCoachLine` 爽鬥版(掙脫/可回收/瓶砸人/拉桿邀請)+ `drawIntro`(主管訓話=世界觀留+「把對手丟進中央回收口 ×3 就贏」) |
