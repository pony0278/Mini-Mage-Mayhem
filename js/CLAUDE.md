...
| `v2-hud.js` | ~380 | 2D HUD(穩定條/道具次數/橫幅/報告)+ **憲章面板 `drawCharter`**(左上雙序列行:名+下班進度 pips+當前組 4 icon(✓/當前脈動)+事故能量條;右上整局時鐘;艙上「投入:🔥」via project)+ `drawBoss`(老闆 emoji 頭像框:intro 訓話/`bossT` 結算宣布)+ `drawPerformLED`(封艙+吐回共用)+ `drawCoachLine` 憲章版(拿對錯/能量滿/找需求) |
