...
| `v2-combat.js` | 379 | 移動(**冰=鎖滑**:帶動量踩上鎖直線滑到撞牆暈/滑進艙,**滑撞另一角色=`slideCollide` 保齡球雙方摔倒**,靜止站上=`ICE_WALK` 小心走;`onSlipperyIce`)、三連擊(punch→`_strikeAt`→`resolveStrike`)、`freezeFighter`(冰凍=暈的冰凍皮,同 STUN_T/restun 規則;被扛保留、放開/丟/掙脫解凍)、精準格擋/推開、抓-搬-掙脫-投擲、收容裁定(三階段)、**地板讀取 `floorHazards`+`drainFloorEvents`**、揍桶/揍總開關、AI |
