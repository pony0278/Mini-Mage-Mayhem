...
export const ITEM_TYPES = ['wind', 'teleport', 'ice', 'oil', 'fire'];
export const ITEM_INFO = { wind: { name: '風壓手套', color: '#bfeaff' }, teleport: { name: '傳送符', color: '#c98cff' }, ice: { name: '冰霜瓶', color: '#9fd8ff' }, oil: { name: '潤滑油瓶', color: '#c9a86a' }, fire: { name: '噴火帽', color: '#ff7a3a' } };
...
export const ITEM_SPEC = {
  ...
  // 油+火(第一批,docs §9.2):油=丟擲瓶(複用冰瓶管線,落地種油膜、不凍人)、火=前方噴流錐(種火地板+點燃油=R1 火海)。
  oil:      { uses: 3, clip: 'barrel_throw', delay: BARREL_THROW_DELAY, whileDisabled: false, aim: 'facing', kind: 'hazard' }, // A 案鋪陳(用量大 ×3)
  fire:     { uses: 2, clip: 'rhook', delay: STRIKE_DELAY[0], whileDisabled: false, aim: 'facing', kind: 'blast' },            // 噴火帽=噴流(rhook 暫代;點燃油海)
...
// 油瓶(丟擲瓶,複用冰瓶管線):同拋物線、落地種油膜(不凍人);油膜可被火點燃=R1 火海、被冰=R5 冰面。
export const OIL_LOB = { range: 180, apex: 34, T: 0.5, h0: 58 };
...
export const itemProjectiles = []; // 拋擲道具投擲物(冰/油瓶;{ x,y,vx,vy,flyT0,z,elem,lob,alive });round reset 清空
export const SLIDE_CONTAIN_V = 200; // 失控入艙:被擊退/打滑速度 > 此值且進艙半徑 = 收容(spec F §2.2)
// 補給座刷新加權(玩家反饋:油出太少)。保底層(油/風=核心鋪面+送人料,常見)> 一般層(冰/火)> 稀有層(傳送=逃脫神器)。
// 機率 = 權重 / 總權重(現值總 11:油27%/風27%/冰18%/火18%/傳送9%)。調各道具出現率只動這張表。
export const ITEM_WEIGHT = { oil: 3, wind: 3, ice: 2, fire: 2, teleport: 1 };
