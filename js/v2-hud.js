...
function drawCoachLine() {
  ...
  // 動態教學(使用者上手文檔 §5):依玩家實際行為即時切提示——攻擊路線(B)一路引到「可回收→拖進艙」;
  // 待機時永遠給核心目標(舊 bug:開場沒事做時提示是空的=頭號困惑源)。
...
/* Route A 清運進度(使用者上手文檔 §9:普通垃圾清運進度,要跟人員回收次數=三格 pip 分開,免混淆勝利條件)。
   本機玩家:一排 CLEANUP_NEED 格 + 「♻ 清運 n/3」,滿了會提示「再回收 1 件取得工具」。 */
function drawCleanupMeter() {
  const n = v2s.cleanup[LOCAL], full = CLEANUP_NEED;
  const size = 12, gap = 4, x0 = 24, y0 = 58;
  hctx.textAlign = 'left'; hctx.font = '800 12px system-ui, sans-serif';
  hctx.fillStyle = 'rgba(154,255,208,.9)'; hctx.fillText('♻ 清運', x0, y0 - 3);
  for (let i = 0; i < full; i++) {
    const px = x0 + 52 + i * (size + gap);
    hctx.fillStyle = i < n ? '#4dffcf' : 'rgba(255,255,255,.14)'; hctx.fillRect(px, y0 - 13, size, size);
    hctx.strokeStyle = 'rgba(154,255,208,.5)'; hctx.lineWidth = 1; hctx.strokeRect(px + .5, y0 - 12.5, size - 1, size - 1);
  }
  hctx.fillStyle = 'rgba(200,235,255,.6)'; hctx.font = '700 10px system-ui, sans-serif';
