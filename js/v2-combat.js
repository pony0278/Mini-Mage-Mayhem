...
import {
  ...
  SLIDE_MIN, SLIDE_KNOCK_V, ICE_WALK, STAGE_NAME, STAGE_BANNER, PERFORM_T, PERFORM_DOME_R, WASTE_CLASS, INTRO_GO, ANGER_HIT, ANGER_MAX,
...
export function resolveStrike(f) { // impact 影格:執行命中掃描+全部打擊回饋
  ...
  for (const o of fighters) {
    ...
    if (!v2s.rampage && o.ai && f.pid === LOCAL) v2s.anger = Math.min(ANGER_MAX, v2s.anger + ANGER_HIT); // 砸 AI 同事 → 怒氣↑↑(速攻路線:激怒=快暴走)
...
function demoMove(f) {
  // 分類事故引擎:AI 同事 stage 0——讀中央口需求 v2s.demand,搶著撿「對的元素」垃圾餵進去(=工作競賽)。
  // 暴走(v2s.rampage)前一直當同事分類;暴走才切戰鬥。被打暈=暫時中斷(醒了繼續分)。
  if (v2s.rampage) { f._aiMode = 'fight'; f._fightSince = game.time; f._aiWantGrab = false; f._aiWantThrow = false; return aiMove(f); }
  ...
  } else {                                                 // 空手 → 找最近的「對的元素」垃圾瓶(只撿分得對的,才不會自己惹自己怒)
    let best = null, bd = 1e9;
    for (const t of bottles) { if (!t.alive || t.held || t.elem !== v2s.demand) continue; const d = Math.hypot(t.x - f.x, t.y - f.y); if (d < bd) { bd = d; best = t; } }
    if (best) { gx = best.x; gy = best.y; if (bd <= GRAB_RANGE + best.r) f._aiWantGrab = true; }
    else { gx = f.x; gy = f.y; }                           // 場上沒有當前需求的垃圾(飛行中/被撿走)→ 原地等下一輪需求
  }
