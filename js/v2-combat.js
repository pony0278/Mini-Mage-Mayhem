...
function demoMove(f) {
  const o = fighters[1 - f.pid];
  const engaged = o.carrying || o.carryObj || (o.punchFx > 0 && game.time - o.punchFx < 0.6); // 玩家搬東西/剛出拳=engage
  if (f.flinchT > 0 || f.stunned || engaged || f._demoThrows >= 2) { f._aiMode = 'fight'; f._fightSince = game.time; f._aiWantGrab = false; f._aiWantThrow = false; return aiMove(f); }
  let gx, gy;
  ...
  if (f.carryObj) {                                        // 扛著垃圾 → 搬到回收口正中央「放」進去(可靠清運;丟太遠會落在艙外)
    ...
    for (const t of bottles) { if (!t.alive || t.held) continue; const d = Math.hypot(t.x - f.x, t.y - f.y); if (d < bd) { bd = d; best = t; } }
    if (best) { gx = best.x; gy = best.y; if (bd <= GRAB_RANGE + best.r) f._aiWantGrab = true; }
    else { gx = f.x; gy = f.y; }                           // 場上沒垃圾(都被回收/飛行中)→ 原地等
  }
