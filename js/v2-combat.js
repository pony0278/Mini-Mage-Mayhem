...
import {
  ...
  STAB_MAX, PUNCH_RANGE, PUNCH_CONE, COMBO_STAB, COMBO_CD, COMBO_WINDOW, STRIKE_DELAY,
...
export function doPerfectParry(d) { // 黃金窗口內按下:取消對方那拳+強推開+自己充能(反暈退役=不繞能量閘;§13 定案 6)
  const a = d.parryFrom;
  ...
  a.vx += Math.cos(ca) * PUSH_FORCE * 1.2; a.vy += Math.sin(ca) * PUSH_FORCE * 1.2; // 攻擊方被彈開
  a.fumbleT = Math.max(a.fumbleT, 0.6);
  addEnergy(d.pid, ENERGY_PARRY);              // 技巧獎勵:黃金窗=充能(反擊資源循環)
  inc.parries++; inc.types.add('parry');
  hitSpark(cpx, cpy, '#fff6c9', 2.4); addRing(cpx, cpy, 42, '#ffe97a', 0.42, 6);
  addText(d.x, d.y - 40, '完美格擋！⚡+能量', '#ffe97a');
...
export function resolveStrike(f) { // impact 影格:執行命中掃描+全部打擊回饋
  ...
  for (const o of fighters) {
    ...
    // 憲章 §3.3:被正式命中=充「被打方」能量(反擊資源);同段連擊只計一次(ENERGY_HIT_CD 防站樁 farm)
    if (game.time - (o._energyHitT || -9) > ENERGY_HIT_CD) { o._energyHitT = game.time; addEnergy(o.pid, ENERGY_HITBY); }
    ...
    // 憲章能量閘(§13 定案 4):擊暈只走「能量滿的第三拳」;穩定值歸零不再自動暈(留給防禦/環境互動)。
    // 免費拳=踉蹌+掉瓶(§3.2 干擾只製造時間差;掉瓶由 v2.js 扛桶 loop 的 fumbleT 條件接手,零新管線)。
    if (fin) {
      if (energized(f.pid) && !o.stunned && o.restunT <= 0) {           // 能量滿的第三拳=擊暈(消耗整條;暈=可抓可回收)
        v2s.energy[f.pid] = 0; f._energyFullShown = false;
        stunFighter(o);
        addText(o.x, o.y - 46, '⚡ 能量重擊！', '#c9a6ff'); addRing(o.x, o.y, 40, '#c9a6ff', 0.4, 6);
      } else {                                                          // 免費第三拳=強推+較長踉蹌(不暈、不打飛=不能免費送艙)
        o.vx += Math.cos(a) * PUSH_FORCE; o.vy += Math.sin(a) * PUSH_FORCE;
        o.fumbleT = Math.max(o.fumbleT, 0.5);
        if (o.carrying) dropCarry(o);
      }
    } else o.fumbleT = Math.max(o.fumbleT, 0.3);                        // 普通鉤拳=短踉蹌(§3.4 普通推開 0.3~0.8s;扛瓶者掉瓶)
  ...
  if (!v2s.stationsArmed && v2s.propsFull) for (const sw of labSwitches) { // 揍左右任一緊急拉桿 → arm 四站(憲章休眠:?props=full 才可拉)
...
export function containByCarry(f, o) { // 拖進艙:憲章 §3.5=招牌干擾演出(捕捉→拒收→北管道吐回≈3s 停工),不再直接得分
  if (v2s.perform || v2s.eject) return;                   // 演出中不疊加
  ...
  startEject(w, o, rev ? 'reverse' : 'carry');
}
export function containByEnviron(v, cause) { // 被擊退/打滑失控進艙 → 同樣走「拒收吐回」干擾(環境喜劇照舊,只是不得分)
  if (v2s.perform || v2s.eject) return;
  ...
  startEject(w, v, rev ? 'reverse' : cause);
}
// --- 中途收容=拒收吐回(憲章 §3.5/§13 定案 2):捕捉+掃描 EJECT_T → 北管道拋回+落地保護。
//     被大招處理的一方 +能量(§3.3 反擊資源)。containLog 照記(報告素材)。 ---
export function startEject(w, v, cause) {
  containLog.push({ winner: w, method: cause, stage: v2s.stage });
  addRing(POD.x, POD.y, POD.r * 1.8, COLORS[w], 0.5, 5); addShake(6);
  v.x = POD.x; v.y = POD.y; v.vx = 0; v.vy = 0; v.z = 0;
  v._thrownT = -9; v._lying = false; v.fumbleT = 0; v._slideVx = 0; v._slideVy = 0;
  v.stunned = true; v.stunT = 99; v.frozen = false; v.invuln = 99; v._performing = true; // 重用演出釘艙機制(v2.js _performing 迴圈)
  if (v.carryObj) { const b = v.carryObj; b.held = false; b.x = POD.x + POD.r + 12; b.y = POD.y + 20; b.vx = 0; b.vy = 0; v.carryObj = null; } // 手上的瓶掉艙邊
  v2s.eject = { pid: v.pid, by: w, t: 0, T: EJECT_T, phase: 'capture', pk: 0, line: '回收目標已捕捉' };
  game.sfx.push('thud');
  dlog('EJECT start', NAMES[v.pid], cause);
}
export function updateEject(dt) {
  const p = v2s.eject; if (!p) return;
  const v = fighters[p.pid];
  v.stunT = 99; p.t += dt; const k = Math.min(1, p.t / p.T);
  if (k < 0.45) { p.phase = 'capture'; p.pk = k / 0.45; p.line = '回收目標已捕捉'; }
  else { p.phase = 'scan'; p.pk = (k - 0.45) / 0.55; p.line = '分類結果:可回收性不足 → 退回'; }
  if (p.t >= p.T) {                                       // 北管道吐回:拋物線+受擊保護(防蹲點)
    v2s.eject = null;
    v._performing = false; v.stunned = false; v.stunT = 0; v.stability = Math.max(v.stability, 50);
    const a = Math.atan2(EJECT_POINT.y - POD.y, EJECT_POINT.x - POD.x);
    const F = EJECT_LOB.range / EJECT_LOB.T;
    v.vx = Math.cos(a) * F; v.vy = Math.sin(a) * F;
    v.fumbleT = EJECT_LOB.T + 0.1; v._thrownT = game.time; v._lob = EJECT_LOB;
    v.invuln = EJECT_LOB.T + EJECT_INVULN;
    addEnergy(v.pid, ENERGY_HITBY);                       // 被大招處理=反擊資源(§3.3)
    addText(POD.x, POD.y - 48, '退回不合格品', '#ffd36d'); addRing(POD.x, POD.y, POD.r * 1.5, '#ffd36d', 0.4, 5);
    game.sfx.push('dash');
    dlog('EJECT spit', NAMES[v.pid]);
  }
}
// --- 下班結算(憲章 §7/§13 定案 5):配額達成或時間到 → 老闆宣布;輸家在艙=封艙壓縮(讀法 B 合體),否則加班 gag ---
export function endShift(w, cause) {
  if (v2s.shiftEnded || v2s.matchOver) return;
  v2s.shiftEnded = true; v2s.winnerPid = w;
  const loser = fighters[1 - w];
  v2s.bossT = 5; v2s.bossLine = cause === 'time' ? '下班鐘響！' + NAMES[w] + ' 完成較多——先走' : NAMES[w] + ' 完成分類配額，提前下班！';
  v2s.bannerText = '⏰ 下班！'; v2s.winBannerT = 1.6;
  addShake(6); game.sfx.push('waveclear');
  dlog('SHIFT END', cause, 'winner', NAMES[w]);
  const inPodNow = (v2s.eject && v2s.eject.pid === loser.pid) || (loser.state === 'alive' && inPod(loser.x, loser.y));
  if (inPodNow) {                                         // 讀法 B:鐘響時人在艙裡 → 拒收改判「正式回收」= 封艙壓縮(perform #3)
    if (v2s.eject) { v2s.eject = null; loser._performing = false; }
    roundWins[w] = WIN_TARGET;                            // 讓 startPerform 走最終封存風味(#3 壓縮北送 → finalSeal → 報告)
    startPerform(w, loser);
  } else {                                                // 加班 gag:輸家留下加班,直接進結算報告
    addText(loser.x, loser.y - 44, '📋 留下加班……', '#9aa5b8');
    addRing(loser.x, loser.y, 36, '#9aa5b8', 0.5, 5);
    endMatch(w);
  }
}
...
export function finalSeal(w) { // 下班封艙儀式(讀法 B:鐘響時輸家在艙裡=正式回收)→ 事故報告
  v2s.bannerText = NAMES[w] + ' 下班！對手已完成回收'; v2s.winBannerT = 3.0;
...
// 示範者 AI(使用者上手文檔 §6):首局開場 AI 先當「上班中的回收員」——撿垃圾瓶、搬到回收口、丟進去
// 示範清運迴圈(讓玩家看到:有個活的對手、東西往艙送、艙是目標)。玩家一 engage(搬東西/攻擊)或 AI 被打→切 fight。
// steer 住這裡(只讀 bottles/POD);實際撿/丟 intent 由 v2.js 執行(v2-combat 不能 import v2-items)。
function demoMove(f) {
  // 憲章 AI=同事+競爭者(§10.2):平時讀「自己的序列進度」分類競速;能量滿且落後 → 切 fight 花能量干擾
  // (拳→能量重擊暈→抓→丟艙=3s 停工),打完/能量花完 → 回來工作。被打暈=暫時中斷(醒了繼續分)。
  if (f.stunned || f.fumbleT > 0 || f.carriedBy) { f._aiWantGrab = false; f._aiWantThrow = false; return { x: 0, y: 0 }; }
  const me = fighters[1 - f.pid];
  const behind = v2s.sets[1 - f.pid] > v2s.sets[f.pid]
    || (v2s.sets[1 - f.pid] === v2s.sets[f.pid] && v2s.seqIdx[1 - f.pid] > v2s.seqIdx[f.pid] + 1); // 落後判定(組數,平手看序列差 2+)
  if (f._aiMode === 'fight') {
    if (energized(f.pid) || f.carrying || me.stunned) return aiMove(f);   // 還沒把能量花掉/正在抓搬 → 打完這一套
    f._aiMode = 'demo';                                                    // 能量花完 → 回去分類
  } else if (energized(f.pid) && behind && !v2s.shiftEnded) {              // 干擾決策:能量滿+落後 → 去鬧玩家
    f._aiMode = 'fight'; f._fightSince = game.time;
    f._aiWantGrab = false; f._aiWantThrow = false;
    return aiMove(f);
  }
  const need = seqNeed(f.pid);
  let gx, gy;
  f._aiWantGrab = false; f._aiWantThrow = false;
  if (f.carryObj) {                                        // 扛著垃圾 → 搬到回收口正中央「放」進去(可靠清運;丟太遠會落在艙外)
    gx = POD.x; gy = POD.y;
    if (Math.hypot(POD.x - f.x, POD.y - f.y) <= 22) f._aiWantThrow = true; // 走到艙心才放(v2.js 用 dropBarrel:落點在艙內=recycle)
  } else if (need) {                                       // 空手 → 找最近的「自己序列需要的」垃圾瓶(§10.3 可預測:AI 走向=它要的)
    let best = null, bd = 1e9;
    for (const t of bottles) { if (!t.alive || t.held || t.elem !== need) continue; const d = Math.hypot(t.x - f.x, t.y - f.y); if (d < bd) { bd = d; best = t; } }
    if (best) { gx = best.x; gy = best.y; if (bd <= GRAB_RANGE + best.r) f._aiWantGrab = true; }
    else { gx = f.x; gy = f.y; }                           // 需要的型不在場(稀缺窗/被搶走)→ 原地等 respawn 保底
  } else { gx = f.x; gy = f.y; }                           // 完賽(endShift 已觸發)
  const dx = gx - f.x, dy = gy - f.y, dl = Math.hypot(dx, dy) || 1;
  const dir = FREEFORM ? aiSafeDir(f, dx / dl, dy / dl) : { x: dx / dl, y: dy / dl };
  if ((dir.x || dir.y) && !f._aiWantThrow) f.facing = Math.atan2(dir.y, dir.x);
  return dir;
}
export function aiMove(f) {
  if (f._aiMode === 'demo') return demoMove(f);
  const o = fighters[1 - f.pid]; // the rival (1v1)
  // 首局容錯(使用者上手文檔 §6):剛從示範切成對手的頭幾秒不主動出拳,給玩家反應/先手空間
  const attackGrace = v2s.tutorial && game.time < (f._fightSince || -9) + 3.0;
  let gx, gy;
