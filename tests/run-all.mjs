...
const SUITES = [
  ...
  ['hudcards',  '下方狀態卡 hud-1(雙卡桌機下方/手機上方/YOU 標/GLB 頭像快照/2D 退路/像素取樣)+ 頭頂標記 ui-1/ui-3(?mark=arrow 風箏箭頭轉向;預設倒三角只標本機、腳下乾淨、持道具才補瞄準箭頭)'],
  ...
  ['skin',      'camp-5 換皮+HUD 層級重整(規格 H §7/§8:標題列=關卡+擋路的人/第二行只在練習模式/目標提示只在第 1 關/動作提示照舊/開場台詞隨關卡/舊框架用語絕跡)'],
  ['magickey',  'camp-3 魔法鑰匙(規格 H §5:過關生成掉落動畫/四節拍走完自清/飛行中先留空落定才點亮/**整排底板=任何背景都讀得到**像素驗收/三把湊齊脈動/加班模式不畫)'],
  ['campaign',  'camp-1 闖關殼(規格 H §3:三關狀態機/危險等級綁關卡/**真終演→掉鑰匙不跳報告**/敗北重打本關鑰匙保留/三把湊齊下班/中離續玩/**free 模式走舊路**=回歸保命符/逃跑退役)'],
  ['menu',      'camp-0 主選單(規格 H §14:流水線工作循環/對手退場/HUD 收掉/開始遊戲交還開場/**自動化預設跳過選單**=其餘 39 支的命脈)'],
