...
| [v2-element-floor-chemistry.md](v2-element-floor-chemistry.md) | **地板化學（元素反應藍圖）**：一格一狀態（取代非疊層）+ ~6 條招牌反應表（複用單機火燃油/雷導水/火引爆毒）+ 地板狀態機 + 兩個已答複合情境 + 場地洩漏掛鉤；**§9 元素×道具對照（7 元素投放原型 + 次數3/2/1 + 四項定案）／§10 元素站洩漏節奏（總開關拉桿啟動、四站錯開輪噴、雷站=原始電弧）** |
