...
> **現行主線=規格 H「三鑰逃生」(2026-08-04 使用者拍板)**:闖關遊戲——打工仔超時加班想下班,
> 三個同事(老油條/領班/老闆)依序擋路,打贏就丟進回收艙、各掉一把魔法鑰匙,湊齊 3 把開門下班。
> **規格 E 的「魔法事故報告」北極星已退役**(使用者:「那是舊的想法」);規格 G 的戰鬥循環原封保留=單關勝利條件。
> 前身:單機 roguelike → 環境處決短局 PvP(規格 A–D 仍為素材引擎)→ 事故報告(規格 E/G)。
...
| [v2-spec-C-share.md](v2-spec-C-share.md) | **規格 C(⚠ 分享文字/挑戰碼已隨事故報告於 camp-2 退役;本文留作設計史)**：最大災難回顧 / 一鍵分享截圖卡（站在 A 鏡頭 + B 歸因上）|
...
| [v2-spec-E-incident-report.md](v2-spec-E-incident-report.md) | **規格 E(⚠ 北極星已於 2026-08-04 由規格 H 取代;報告系統退役,本文留作紀錄)**：魔法事故報告/收容測試；吸收 C 為核心；收容狀態機 + 報告 schema + 等級 + V0.8/0.9/1.0 路線；可重用零件對照 |
| [campaign.md](campaign.md) | **闖關化決策(2026-07-16,未動工;已被規格 H 收斂取代——前身紀錄)**:3 關+廠長 boss+逃出下班——關卡表/boss 三段擊倒設計/台詞草稿/實作計畫/不做清單;戰鬥系統不動,闖關=外殼 |
...
| [v2-spec-H-three-keys-escape.md](v2-spec-H-three-keys-escape.md) | **規格 H(現行主線,2026-08-04 拍板)**:三鑰逃生闖關——病因診斷(「提示很多但很亂」=目標本身要解釋)、**世界留著只換任務**(工廠/艙口 LED/地板標語全不動)、關卡狀態機(fight→order→perform→keydrop→handoff→escape)、三 boss `AI_PROFILE` 加列(**不給老闆特殊規則**)、魔法鑰匙自動飛入、**事故報告精確切除清單**、HUD 資訊層級重整、測試遷移(40 支中 10 支受影響、只有 4 個中文硬釘)、⚠ 別重用 `v2s.stage` |
