...
- **結算/報告畫面補觸控按鈕**(再戰/複製;原本靠鍵盤 R/C)——✅ 已做(觸控保底出口:
  matchOver 時亮「再戰/複製」並收起對戰控制;動作回呼 `restartMatch`/`copyShare`,
  不走 step 輪詢因報告畫面 sim 已凍結。健檢 hook `__touch.report()` + `__v2.endMatch`)。
