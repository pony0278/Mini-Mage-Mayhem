...
| `run_cycle` | 跑步循環(雙擊跑 `e.running` 時**循環播放**;拿掉 entry 即退回程序跑姿 `ANIM.run`) | ✅ 使用者 v2(腿擺幅加大+踝角;接入慣例:run_3=run_1、循環段 lin、lags 歸零——studio 匯出常帶舊殘差,每次接入都要套) |

`run_cycle` 編排注意(**第一個循環播放槽**,規則跟一次性動作不同):
- **tag `run` = 循環起點**:0→`run` 是「起跑」過渡段(站姿加速進入,只播一次),
  `run`→**最後一個實排 key** 無縫繞圈(prepClip 自動補的回-idle 收尾段**不進循環**,`lastKeyT` 排除——
  編排時不用管收尾)。**`run` 幀與最後 key 姿勢要「完全」一致**(複製貼上,別重擺:kx 差 40° 就是每圈一跳);
  起跑段自由發揮。沒標 tag → 整條循環。
- **循環段 ease 用 `lin`**(等速流過去;`out`=每步衝到位急停,循環起來像連續煞車)。起跑那段可用 `out`。
- **`lags` 全 0**:循環一圈可能比 lag 還短(0.1s lag=整圈相位),且繞回點會取樣到起跑段=每圈抽動。
- **相位吃位移**:循環段一圈=`ANIM.runClip.stridePx`(現值 108;跑速 269px/s ≈ 0.4s 一圈=左右各一步)。
  studio 裡編幾幀都行,遊戲端按位移縮放——**stridePx 就是播放速度**(擺動頻率=移速÷stridePx),
  想調快慢只動它,不用重排;但注意它和滑步率耦合(下一條)。
- **⚠ 滑步(滑行感)= stridePx 沒對齊腳的觸地掃程**:掃程 ≈ 2×2×腿長14×sin(髖擺幅)——
  髖 ±60° ≈ 48px/圈。stridePx 96 時滑步率 50%(像溜冰,踩過的坑);60 ≈ 20%(風格化可接受)。
  想要「大步幅+慢步頻」:studio 加大髖擺/觸地幀 `lX_stretch` 伸腿 → 掃程變大 → stridePx 跟著調大。
- **踩地感彈跳=遊戲端疊加**(`ANIM.runClip.bob`,0=關):循環相位驅動 root_py——key(觸地)低、
  兩 key 中間(過渡)高=每步一跳,起跑段不疊;clip 不用排 root_py。`?tune=1`「跑步(即時)」
  滑桿邊跑邊調 bob/stridePx,📋 複製貼回 ANIM.runClip。
